...
class ConfigManager:
    ...
    def show_migration_dialog_if_needed(self, parent_widget: Optional[Any] = None) -> None:
    ...
    def get_all_values_from_config(self, key: str, default_list: List[int]) -> List[int]:
    ...
    def _get_config_parser(self) -> configparser.ConfigParser:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                logger.warning(f"Warning: Could not parse existing config file {self.config_file}. Error: {e}")
        return config
    ...
    def load_settings(self) -> Dict[str, Any]:
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                settings["tray_enabled"] = config.getboolean('DEFAULT', keys.TRAY_ENABLED, fallback=False)
                settings["tray_click_opens_cables"] = config.getboolean(
                    'DEFAULT', keys.TRAY_CLICK_OPENS_CABLES, fallback=True
                )
                settings["remember_settings"] = config.getboolean('DEFAULT', keys.REMEMBER_SETTINGS, fallback=False)
                settings["restore_only_minimized"] = config.getboolean('DEFAULT', keys.RESTORE_ONLY_MINIMIZED, fallback=False)

                has_saved_quantum = config.has_option('DEFAULT', keys.SAVED_QUANTUM)
                has_saved_sample_rate = config.has_option('DEFAULT', keys.SAVED_SAMPLE_RATE)
                settings["has_saved_quantum"] = has_saved_quantum
                settings["has_saved_sample_rate"] = has_saved_sample_rate

                if has_saved_quantum:
                    settings["saved_quantum"] = config.getint('DEFAULT', keys.SAVED_QUANTUM, fallback=0)
                if has_saved_sample_rate:
                    settings["saved_sample_rate"] = config.getint('DEFAULT', keys.SAVED_SAMPLE_RATE, fallback=0)

                settings["autostart_enabled"] = config.getboolean('DEFAULT', keys.AUTOSTART_ENABLED, fallback=False)
                settings["check_updates_at_start"] = config.getboolean('DEFAULT', keys.CHECK_UPDATES_AT_START, fallback=False)

                appimage_path = config.get('DEFAULT', keys.APPIMAGE_PATH, fallback=None)
                if appimage_path and not os.path.exists(appimage_path):
                    logger.warning(f"Warning: Configured AppImage path does not exist: {appimage_path}")
                    appimage_path = None
                settings["appimage_path"] = appimage_path

                logger.info(f"Loaded tray_click_opens_cables from config: {settings['tray_click_opens_cables']}")
                logger.info(f"Loaded remember_settings: {settings['remember_settings']}")
                logger.info(f"Loaded autostart_enabled: {settings['autostart_enabled']}")
                logger.info(f"Loaded check_updates_at_start: {settings['check_updates_at_start']}")
                logger.info(f"Loaded restore_only_minimized: {settings['restore_only_minimized']}")
    ...
    def save_settings(self, settings_dict: Dict[str, Any]) -> None:
        ...
        config['DEFAULT'].update({
            ...
            keys.TRAY_CLICK_OPENS_CABLES: str(settings_dict.get('tray_click_opens_cables', True)),
            keys.REMEMBER_SETTINGS: str(settings_dict.get('remember_settings', False)),
            keys.RESTORE_ONLY_MINIMIZED: str(settings_dict.get('restore_only_minimized', False)),
            keys.AUTOSTART_ENABLED: str(settings_dict.get('autostart_enabled', False)),
            keys.CHECK_UPDATES_AT_START: str(settings_dict.get('check_updates_at_start', False)),
            keys.APPIMAGE_PATH: str(settings_dict.get('appimage_path') or ''),
        })

        for key in [keys.TRAY_ENABLED, keys.TRAY_CLICK_OPENS_CABLES, keys.REMEMBER_SETTINGS,
                   keys.RESTORE_ONLY_MINIMIZED, keys.AUTOSTART_ENABLED, keys.CHECK_UPDATES_AT_START, keys.APPIMAGE_PATH]:
            self._changed_keys.add(key)

        self._mark_dirty()
        self.flush()

    def toggle_remember_settings(self, remember: bool, current_quantum: Optional[int] = None,
                                  current_sample_rate: Optional[int] = None,
                                  quantum_was_reset: bool = False, sample_rate_was_reset: bool = False,
                                  tray_enabled: bool = False, tray_click_opens_cables: bool = True) -> Dict[str, Any]:
        ...
        if remember:
            ...
            if current_sample_rate and not sample_rate_was_reset:
                ...
                logger.info(f"Remember settings: Saved sample rate {current_sample_rate}")
    ...
    def ensure_config_lists(self) -> None:
        ...
        if not config.has_option('DEFAULT', keys.QUANTUM_VALUES):
            config['DEFAULT'][keys.QUANTUM_VALUES] = ','.join(str(x) for x in self.DEFAULT_QUANTUM_VALUES)
            self._changed_keys.add(keys.QUANTUM_VALUES)
            config_updated = True

        if not config.has_option('DEFAULT', keys.SAMPLE_RATE_VALUES):
            config['DEFAULT'][keys.SAMPLE_RATE_VALUES] = ','.join(str(x) for x in self.DEFAULT_SAMPLE_RATE_VALUES)
    ...
    def _save_audio_setting(self, setting_name: str, config_key: str, current_value: Optional[int],
                            was_reset: bool) -> None:
        ...
        try:
            ...
            if current_value and current_value != "Edit List...":
                ...
                if config.has_option('DEFAULT', config_key):
                    ...
                    logger.info(f"Removed invalid/empty {setting_name} setting ({config_key}) from config")
    ...
    def save_quantum_setting(self, current_value: Optional[int] = None, was_reset: bool = False) -> None:
    ...
    def save_sample_rate_setting(self, current_value: Optional[int] = None, was_reset: bool = False) -> None:
