...
class EmbeddedSettingsPanel(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.restore_minimized_checkbox = QCheckBox(
            "Restore above only when auto-started"
        )
        self.restore_minimized_checkbox.stateChanged.connect(
            self._on_restore_minimized_changed
        )
        ...
        self.apply_immediately_checkbox.stateChanged.connect(
            self._on_apply_immediately_changed
        )
        ...
        basic_layout.addWidget(self.show_confirmation_checkbox)
        ...
        apply_button = self.other_button_box.button(
            QDialogButtonBox.StandardButton.Apply
        )
        self.builder.build_widgets(
            other_layout, label_width=180, apply_button=apply_button
        )
        ...
        coffee_label.setText(
            '<a href="https://buymeacoffee.com/magillos" style="color: #FF813F; text-decoration: none; font-weight: bold;">☕ Buy me a coffee</a>'
        )
    ...
    def _load_settings(self) -> None:
        ...
        self.apply_immediately_checkbox.setChecked(self.app.apply_immediately)
        self.apply_immediately_checkbox.blockSignals(False)

        self.show_confirmation_checkbox.blockSignals(True)
        self.show_confirmation_checkbox.setChecked(self.app.show_confirmation)
    ...
    def _on_remember_changed(self, state: int) -> None:
        self.app.remember_settings_checkbox.setChecked(
    ...
    def _on_restore_minimized_changed(self, state: int) -> None:
        self.app.restore_only_minimized_checkbox.setChecked(
            state == Qt.CheckState.Checked.value
        )

    def _on_apply_immediately_changed(self, state: int) -> None:
        self.app.apply_immediately_checkbox.setChecked(
            state == Qt.CheckState.Checked.value
        )

    def _on_show_confirmation_changed(self, state: int) -> None:
        self.app.show_confirmation_checkbox.setChecked(
            state == Qt.CheckState.Checked.value
        )
    ...
    def _update_version_label(self) -> None:
        ...
        if self.app.update_manager.update_available:
            ...
            self.version_label.setText(
                f'<a href="https://github.com/magillos/Cable/releases" style="color: orange; text-decoration: none;">Version: {curr_version} (Update: {latest})</a>'
            )
        else:
            self.version_label.setText(
                f'<a href="https://github.com/magillos/Cable/releases" style="color: grey; text-decoration: none;">Version: {curr_version}</a>'
            )
