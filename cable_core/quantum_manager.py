...
class QuantumManager:
    ...
    def __init__(
        ...
        self.sample_rate_combo.currentIndexChanged.connect(self._on_sample_rate_index_changed)
    ...
    def _populate_quantum_dropdown(self) -> None:
        ...
        try:
            ...
            values = self.config_manager.get_list_from_config('quantum_values', self.DEFAULT_QUANTUM_VALUES)
            for value in values:
    ...
    def _populate_sample_rate_dropdown(self) -> None:
        ...
        try:
            ...
            values = self.config_manager.get_list_from_config('sample_rate_values', self.DEFAULT_SAMPLE_RATE_VALUES)
            for value in values:
    ...
    def _set_combo_to_value(self, combo: QComboBox, value_str: str, last_valid_attr: str) -> None:
        ...
        if index >= 0:
            ...
            if edit_item_index >= 0:
                ...
                logger.warning(f"Could not find '{EDIT_LIST_TEXT}' to insert '{value_str}' before.")
    
    def load_current_settings(self, force_reset_quantum: bool = False, force_reset_sample_rate: bool = False) -> None:
        """
    ...
    def apply_loaded_settings(self, settings: Dict[str, Any]) -> None:
        ...
        force_reset_sample_rate = getattr(self, '_pending_force_reset_sample_rate', False)
        ...
        try:
            ...
            if sample_rate:
                self._set_combo_to_value(self.sample_rate_combo, sample_rate, 'last_valid_sample_rate_index')
            
            if quantum:
                self._set_combo_to_value(self.quantum_combo, quantum, 'last_valid_quantum_index')
    ...
    def apply_quantum_settings(self, skip_save: bool = False, remember_settings: bool = False) -> bool:
        """
        ...
        if result.get("ok"):
            ...
            if new_index >= 0:
                ...
                logger.debug(f"Updated last valid index for quantum to {new_index} after applying '{value_str}'")
            return True
        else:
            ui = result.get("ui", {})
    ...
    def apply_sample_rate_settings(self, skip_save: bool = False, remember_settings: bool = False) -> bool:
        """
        ...
        if result.get("ok"):
            data = result.get("data", {})
            self.sample_rate_was_reset = data.get("was_reset", self.sample_rate_was_reset)
            new_index = self.sample_rate_combo.findText(value_str)
            if new_index >= 0:
                self.last_valid_sample_rate_index = new_index
                logger.debug(f"Updated last valid index for sample rate to {new_index} after applying '{value_str}'")
            return True
        else:
            ui = result.get("ui", {})
    ...
    def _edit_value_list(self, title: str, config_key: str, default_values_list: List[int]) -> None:
        ...
        # Show the dialog
        dialog = ValueSelectorDialog(f"Select Active {title} Values", all_values, active_values, self.parent)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            selected_values = dialog.get_selected_values()
            logger.debug(f"Dialog accepted for {title}. Selected values: {selected_values}")
