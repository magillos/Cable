...
from PyQt6.QtWidgets import (
    QVBoxLayout, QHBoxLayout, QLabel, QSlider, QFrame,
    QLineEdit, QCheckBox, QMessageBox, QWidget, QLayout
)
...
class SettingsWidgetBuilder:
    ...
    def build_widgets(self, target_layout: QLayout, label_width: Optional[int] = None) -> None:
        ...
        for key, setting_info in SETTINGS_MAP.items():
            ...
            slider.valueChanged.connect(lambda v, k=key: self.value_labels[k].setText(str(v)))
        ...
        untangle_label.setFixedWidth(label_width)
        ...
        self.auto_layout_split_checkbox = QCheckBox("Split nodes in Auto layout in Graph")
        self.auto_layout_split_checkbox.setToolTip("When ON, Auto layout automatically splits nodes with connections on both sides.\n"
        ...
        self.colored_connections_checkbox.setToolTip("Use per-client colors for connection lines\n"
    ...
    def load_settings(self) -> None:
        ...
        self.midi_matrix_checkbox.setChecked(self.config_manager.get_bool(keys.ENABLE_MIDI_MATRIX, False))
        self.straight_lines_checkbox.setChecked(self.config_manager.get_bool(keys.USE_STRAIGHT_LINES, False))
        self.split_audio_midi_checkbox.setChecked(self.config_manager.get_bool(keys.GRAPH_SPLIT_AUDIO_MIDI_CLIENTS, False))
        self.auto_layout_split_checkbox.setChecked(self.config_manager.get_bool(keys.GRAPH_AUTO_LAYOUT_SPLIT, False))
        self.colored_connections_checkbox.setChecked(self.config_manager.get_bool(keys.GRAPH_COLORED_CONNECTIONS, False))
        self.verbose_output_checkbox.setChecked(self.config_manager.get_bool(keys.VERBOSE_OUTPUT, False))
        self.integrate_checkbox.setChecked(self.config_manager.get_bool(keys.INTEGRATE_CABLE_AND_CABLES, False))
    ...
    def save_settings(self) -> None:
        ...
        self.config_manager.set_bool(keys.ENABLE_MIDI_MATRIX, self.midi_matrix_checkbox.isChecked())
        self.config_manager.set_bool(keys.USE_STRAIGHT_LINES, self.straight_lines_checkbox.isChecked())
        self.config_manager.set_bool(keys.GRAPH_SPLIT_AUDIO_MIDI_CLIENTS, self.split_audio_midi_checkbox.isChecked())
        self.config_manager.set_bool(keys.GRAPH_AUTO_LAYOUT_SPLIT, self.auto_layout_split_checkbox.isChecked())
        self.config_manager.set_bool(keys.GRAPH_COLORED_CONNECTIONS, self.colored_connections_checkbox.isChecked())
        self.config_manager.set_bool(keys.VERBOSE_OUTPUT, self.verbose_output_checkbox.isChecked())
        self.config_manager.set_bool(keys.INTEGRATE_CABLE_AND_CABLES, self.integrate_checkbox.isChecked())
