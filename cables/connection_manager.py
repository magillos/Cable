...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QTabWidget, QVBoxLayout,
                            ...
                            QSpacerItem, QMessageBox, QTreeWidget, QTreeWidgetItem, QToolBar, QToolButton)
from PyQt6.QtCore import Qt, QMimeData, QPointF, QRectF, QTimer, QSize, QRect, QProcess, QPoint, pyqtSlot, QObject
...
class JackConnectionManager(QMainWindow):
    ...
    def _post_init_cleanup(self) -> None:
        ...
        orphaned_count = self.unified_sink_manager.cleanup_orphaned_unified_sinks(self._jack_service.get_ports())
        if orphaned_count > 0:
    ...
    def _recreate_virtual_sinks_if_autostart(self) -> None:
        ...
        try:
            data = json.loads(
                self.config_manager.get_str(keys.VIRTUAL_SINKS_RECREATE_AT_AUTOSTART, '{}') or '{}'
            )
        ...
        for _client_key, sink_info in data.items():
            ...
            if not sink_name:
                logger.warning(f"Skipping autostart sink entry with no sink_name: {_client_key}")
                continue
            try:
                ...
                if already_exists:
                    logger.info(f"Virtual sink '{sink_name}' already exists, skipping creation.")
                    # Update module ID in config for unload tracking
                    module_id = self.unified_sink_manager.find_module_id_for_external_sink(sink_name)
                    if module_id:
                ...
                result = subprocess.run(
                    ['pactl', 'load-module', 'module-null-sink',
                     f'sink_name={sink_name}', f'channel_map={channel_map}'],
                    capture_output=True, text=True, check=True
                )
                module_id = result.stdout.strip()
                self._update_virtual_sink_module_id(sink_name, module_id)
                logger.info(f"Recreated virtual sink '{sink_name}' (module {module_id})")
    ...
    def _handle_unified_port_added(self, port_name: str, client_name: str, flags: int, type_str: str, is_input: bool) -> None:
        ...
        for node in unified_nodes:
            if node.client_name == client_name:
                logger.debug(f"New port {port_name} for unified client {client_name}. Reconnecting to sink.")
                ...
                for conn_item in list(port_item.connections):
                    if conn_item and conn_item.source_port and conn_item.dest_port:
                        try:
                            ...
                            logger.error(f"Error breaking output connection for {port_item.port_name} of {node.client_name}: {e}")
    ...
    def _restore_window_geometry(self) -> None:
        ...
        if saved:
            ...
            self.resize(app_config.CONN_MANAGER_INITIAL_WIDTH,
    ...
    def _connect_action_manager_signals(self) -> None:
        ...
        # Refresh request
        self.action_manager.refresh_requested.connect(lambda: self.refresh_ports(from_shortcut=True))
        
        # Zoom request
        self.action_manager.zoom_requested.connect(self.tab_manager.handle_zoom_request)
        
        # Tab switch request
        self.action_manager.tab_switch_requested.connect(self.tab_manager.handle_tab_switch_request)
    ...
    def refresh_ports(self, refresh_all: bool = False, from_shortcut: bool = False) -> None:
        if refresh_all:
    ...
    def _on_port_registered(self, port_name: str, is_input: bool) -> None:
        ...
        if (self.latency_tester is not None and
            ...
            logger.debug(f"Detected registration of {port_name}, attempting latency auto-connection via LatencyTester...")
    ...
    def make_connection_selected(self) -> None:
        ...
        if not selected_inputs or not selected_outputs:
            logger.debug("Make Connection: Select at least one input and one output item (port or group).")
            return

        logger.debug(f"Making connections (button): Outputs={selected_outputs}, Inputs={selected_inputs}")
    ...
    def make_midi_connection_selected(self) -> None:
        ...
        if not selected_inputs or not selected_outputs:
            logger.debug("Make MIDI Connection: Select at least one input and one output item (port or group).")
            return

        logger.debug(f"Making MIDI connections (button): Outputs={selected_outputs}, Inputs={selected_inputs}")
        self.jack_handler.make_multiple_connections(selected_outputs, selected_inputs)
    ...
    def break_connection_selected(self) -> None:
        ...
        if not selected_inputs or not selected_outputs:
            logger.debug("Break Connection: Select at least one input and one output port.")
            return

        logger.debug(f"Breaking connections for: Outputs={selected_outputs}, Inputs={selected_inputs}")
    ...
    def break_midi_connection_selected(self) -> None:
        ...
        if not selected_inputs or not selected_outputs:
            logger.debug("Break MIDI Connection: Select at least one input and one output MIDI port.")
            return

        logger.debug(f"Breaking MIDI connections for: Outputs={selected_outputs}, Inputs={selected_inputs}")
        for out_port in selected_outputs:
            for in_port in selected_inputs:
    ...
    def update_connection_buttons(self) -> None:
        self._update_port_connection_buttons(self.input_tree, self.output_tree,
                                           ...
                                           disconnect_action=getattr(self.action_manager, 'audio_disconnect_action', None))
    ...
    def update_midi_connection_buttons(self) -> None:
        self._update_port_connection_buttons(self.midi_input_tree, self.midi_output_tree,
                                           ...
                                           disconnect_action=getattr(self.action_manager, 'midi_disconnect_action', None))
    ...
    def _update_port_connection_buttons(
        ...
        if ports_selected:
            ...
            is_midi = input_tree in (getattr(self, 'midi_input_tree', None),) or output_tree in (getattr(self, 'midi_output_tree', None),)
            ...
            is_group_to_group = len(selected_output_ports) > 1 and len(selected_input_ports) > 1
    ...
    def toggle_graph_fullscreen(self) -> None:
        ...
        if graph_tab_index == -1 or self.ui_manager.tab_widget.currentWidget() != self.ui_manager.graph_tab_widget:
            logger.debug("Graph fullscreen toggle requested, but graph tab is not active or not found.")
            return
