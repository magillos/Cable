...
class NodeVisibilityManager:
    ...
    def __init__(self, connection_manager: 'NodeVisibilityManagerInterface', config_manager: 'ConfigManager') -> None:
        """
    ...
    def is_node_visible(self, node_name: str, is_midi: bool = False) -> bool:
        """
        ...
        if is_midi:
            # For MIDI nodes, check if EITHER inputs or outputs should be visible
            # Default to True if not explicitly set
            input_visible = self.midi_input_visibility.get(client_name, True)
            output_visible = self.midi_output_visibility.get(client_name, True)
            
            # Use OR logic - if either part is visible, the node is visible
            return input_visible or output_visible
        else:
            # For audio nodes, same logic
            # Default to True if not explicitly set
            input_visible = self.audio_input_visibility.get(client_name, True)
            output_visible = self.audio_output_visibility.get(client_name, True)
            
            # Use OR logic - if either part is visible, the node is visible
            return input_visible or output_visible
    
    def is_input_visible(self, node_name: str, is_midi: bool = False) -> bool:
        """
    ...
    def is_output_visible(self, node_name: str, is_midi: bool = False) -> bool:
        """
    ...
    def show_configuration_dialog(self, parent: Optional[QWidget] = None, tab_type: str = 'graph') -> None:
        """
    ...
    def unhide_all_nodes(self) -> None:
        """Reset all visibility settings to show all nodes."""
    ...
    def apply_visibility_settings(self) -> None:
        ...
        if scene:
            ...
            if not hasattr(scene, 'node_visibility_manager') or scene.node_visibility_manager is None:
            ...
            for client_name in set(list(self.audio_input_visibility.keys()) +
                ...
                if ((audio_in_visible != audio_out_visible) or
                    ...
                    logger.debug(f"Node {client_name} has partial visibility - should be split")
                    logger.debug(f"  Audio in: {audio_in_visible}, Audio out: {audio_out_visible}")
                    logger.debug(f"  MIDI in: {midi_in_visible}, MIDI out: {midi_out_visible}")
    ...
    def _auto_split_nodes_for_visibility(self, scene, nodes_to_split: List[str]) -> None:
        """
        ...
        for client_name in nodes_to_split:
            ...
            if not has_inputs or not has_outputs:
                logger.debug(f"Node {client_name} cannot be split: missing {'inputs' if not has_inputs else 'outputs'}")
                continue
                
            # Check if the node has a split_handler
            if not hasattr(node, 'split_handler'):
                logger.warning(f"Node {client_name} has no split_handler, cannot auto-split")
                continue
    ...
    def _update_tree_visibility(self, input_tree: Any, output_tree: Any, is_midi: bool = False) -> None:
        """
