...
class _MatrixGridWidget(QWidget):
    ...
    def _calculate_sizes_from_text(self) -> None:
        ...
        self.output_client_port_counts = {
            client: len(ports) for client, ports in output_client_groups.items()
        }

        # Force uniform square cells for the matrix grid.
        # This matches the intended classic matrix look (as it appeared previously
        # when text measurement implicitly failed) and prevents wide rectangles.
        cell_size = int(35 * self.grid_cell_scaling)
        ...
        for _ in self.parent_matrix.model.output_ports:
            self.row_heights.append(cell_size)
            self.row_positions.append(int(current_y))
            current_y += cell_size
        ...
        for _ in self.parent_matrix.model.input_ports:
            self.column_widths.append(cell_size)
            self.column_positions.append(int(current_x))
            current_x += cell_size
    ...
    def paintEvent(self, event: Optional["QEvent"]) -> None:
        ...
        grid_rect = QRect(
            self.left_margin,
            self.top_margin,
            int(max(available_width, grid_width)),
    ...
    def _draw_grid(self, painter: QPainter, grid_rect: QRect) -> None:
        ...
        if self.column_positions and self.column_widths:
            grid_right_x = int(self.column_positions[-1] + self.column_widths[-1])
    ...
    def _draw_connection_squares(self, painter: QPainter, grid_rect: QRect) -> None:
        ...
        for col, (input_client, input_port, input_display_name) in enumerate(
            ...
            for row, (output_client, output_port, output_display_name) in enumerate(
                ...
                square_width = int(min(
                    column_width - 2, grid_rect.width() - col_x + grid_rect.left() - 2
                ))
                rect = QRect(int(col_x + 1), int(row_y + 1), square_width, int(row_height - 2))
...
class _OutputLabelsWidget(QWidget):
    ...
    def _calculate_truncation_lengths(self) -> None:
        ...
        client_fm = QFontMetrics(client_font)
        ...
        while low <= high:
            ...
            if mid == 0:
                low = 1
                continue
            rect = client_fm.boundingRect("A" * mid)
        ...
        port_fm = QFontMetrics(port_font)
        ...
        while low <= high:
            ...
            if mid == 0:
                low = 1
                continue
            rect = port_fm.boundingRect("A" * mid)
            if rect.width() <= available_width:
                max_chars = mid
                low = mid + 1
            else:
