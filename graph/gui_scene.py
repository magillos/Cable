...
class JackGraphScene(QGraphicsScene):
    ...
    scene_connections_changed = (
        pyqtSignal()
    )  # Signal for when connections are added/removed
    scene_fully_loaded = (
        pyqtSignal()
    )  # Signal emitted when the scene is fully loaded initially
    node_states_changed = (
        pyqtSignal()
    )  # Signal emitted when node positions/split/fold states change due to user action
    ...
    def __init__(
        ...
        self.node_configs, self.initial_zoom_level = (
            self.node_config_manager.load_node_states()
        )

        # Extract the untangle setting from the config dict if present
        self.initial_untangle_setting = self.node_configs.pop(
            self.node_config_manager.CURRENT_UNTANGLE_SETTING_KEY, None
        )
    ...
    def _pop_pending_position_for_client(self, client_name: str) -> QPointF | None:
        ...
        logger.debug(
            f"Looking for pending position for client: {client_name}, pending: {list(self._pending_node_positions.keys())}"
        )
        for sink_name, pos in list(self._pending_node_positions.items()):
            # Exact match
            if client_name == sink_name:
                logger.info(
                    f"Found pending position for exact match: {sink_name} -> {pos}"
                )
                self._pending_node_positions.pop(sink_name, None)
                return pos
            ...
            if client_name.startswith(sink_name + " "):
                logger.info(
                    f"Found pending position for prefix match with space: {sink_name} -> {pos}"
                )
                self._pending_node_positions.pop(sink_name, None)
                return pos
            ...
            if client_name.startswith(sink_name):
                ...
                if (
                    rest
                    and (rest[0] == "-" or rest[0] == ".")
                    and len(rest) > 1
                    and rest[1:].isdigit()
                ):
                    logger.info(
                        f"Found pending position for module ID suffix match: {sink_name} -> {pos}"
                    )
                    self._pending_node_positions.pop(sink_name, None)
                    return pos
    ...
    def _apply_pending_position_if_any(self, node: "NodeItem") -> bool:
        ...
        logger.info(
            f"Applying pending position {pending_pos} to node {node.client_name}"
        )
    ...
    def _on_full_sync_ready(
        self, clients_to_process: dict, present_client_names: set, all_ports: list
    ) -> None:
    ...
    def _apply_node_sync(
        self, clients_to_process: dict, present_client_names: set
    ) -> None:
        ...
        for client_name, client_info in sorted(clients_to_process.items()):
            ...
            if client_name in self.nodes:
                ...
                node = self.add_node(
                    client_name, ports_to_process, original_client_name
                )
                if node:
                    ...
                    if self.layouter and not node.is_split_origin:
                        QTimer.singleShot(
                            0, lambda n=node: self._apply_push_away_for_node(n)
                        )
                    ...
                    if (
                        ...
                        and not node.is_split_origin
                        and not node.is_split_part
                    ):
                    ...
                    preset_handler = getattr(
                        self.connection_manager, "preset_handler", None
                    )
                    if preset_handler:
        ...
        for client_name, node in self.nodes.items():
            ...
            if (
                ...
                if node.isVisible():
                    ...
                    if not config or (
                        "pos" not in config and not config.get("is_split")
                    ):
                        node.setPos(QPointF(20, 20 + new_node_y_offset))
    ...
    def filter_nodes(self, filter_text: str) -> None:
        ...
        exclude_terms = [
            term[1:] for term in terms if term.startswith("-") and len(term) > 1
        ]
    ...
    def _apply_push_away_for_node(
        self, node: "NodeItem", animate: Optional[bool] = None
    ) -> None:
        ...
        victims = self.layouter.get_overlapping_nodes(
            node, current_pos.x(), current_pos.y()
        )
        ...
        for victim in victims:
            ...
            new_x, new_y = self.layouter.find_non_overlapping_position(
                victim, v_pos.x(), v_pos.y()
            )
    ...
    def _animate_node_to_position(
        self, node: "NodeItem", target_x: float, target_y: float
    ) -> None:
    ...
    def add_node(
        self,
        client_name: str,
        client_ports: Optional[Dict[str, jack.Port]] = None,
        ...
        if self.node_visibility_manager:
            ...
            if not self.node_visibility_manager.is_node_visible(
                client_name, is_midi=is_midi, tab_type="graph"
            ):
                return None
        ...
        try:
            ...
            node = NodeItem(
                client_name,
                self.graph_jack_handler,
                self.node_config_manager,
                ports_to_add=client_ports,
    ...
    def remove_node(self, client_name: str, unload_unified_sinks: bool = False) -> None:
        ...
        if node:
            ...
            if node.is_input_unified or node.is_output_unified:
                ...
                logger.debug(
                    f"Preserved unified state in node_configs for {client_name}"
                )
            ...
            if unload_unified_sinks:
                ...
                if node.is_input_unified:
                    logger.debug(
                        f"Unloading input unified sink for node {client_name} before removal"
                    )
                    try:
                ...
                if node.is_output_unified:
                    logger.debug(
                        f"Unloading output unified sink for node {client_name} before removal"
                    )
                    try:
                        if node.unify_handler:
            ...
            if node.is_split_origin:
                ...
                ports_to_clean = list(node.input_ports.values()) + list(
                    node.output_ports.values()
                )
                ...
                if input_part:
                    ...
                    input_ports_to_clean = list(input_part.input_ports.values()) + list(
                        input_part.output_ports.values()
                    )
                ...
                if output_part:
                    ...
                    output_ports_to_clean = list(
                        output_part.input_ports.values()
                    ) + list(output_part.output_ports.values())
                ...
                for port in ports_to_clean:
                    node.remove_port(
                        port.port_name
                    )  # Clean internal refs and visual item
                self.removeItem(node)
    ...
    def find_port_item(self, port_name: str) -> PortItem | None:
        ...
        if not port_obj:
            logger.debug(
                f"find_port_item: Could not get port object for '{port_name}'."
            )
            return None
        ...
        if node_to_search.is_split_origin:
            if port_obj.is_input and node_to_search.split_input_node:
                ...
                logger.debug(
                    f"find_port_item: Port '{port_name}' not found on expected split part of '{node_to_search.client_name}'."
                )
                return None

        # Search for the port on the determined target node
        port_item = target_node.input_ports.get(
            port_name
        ) or target_node.output_ports.get(port_name)
        return port_item
    ...
    def mousePressEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        if self.interaction_handler._moved_node and isinstance(
            self.interaction_handler._moved_node, NodeItem
        ):
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        # If handler has a potential drag item (pressed on port/bulk area), don't start rubber band
        if self.interaction_handler._potential_drag_item is not None:
            event.accept()
            return
    ...
    def mouseReleaseEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        ...
        # (finalizing moves, selection changes).
            ...
            if self.layouter:
                ...
                for aggressor in aggressor_nodes:
                    ...
                    victims = self.layouter.get_overlapping_nodes(
                        aggressor, current_pos.x(), current_pos.y()
                    )
        ...
        if node_that_moved and isinstance(node_that_moved, NodeItem):
            ...
            if (
                self._node_press_pos is not None
                and node_that_moved.scenePos() != self._node_press_pos
            ):
        ...
        for item in self.selectedItems():
            if (
                isinstance(item, NodeItem) and item != node_that_moved
            ):  # Avoid double update
    ...
    def save_node_states(
        self,
        graph_zoom_level: Optional[float] = None,
        current_untangle_setting: Optional[int] = None,
    ) -> None:
        ...
        self.node_config_manager.save_node_states(
            self.nodes,
    ...
    def request_specific_node_save(self, node_item: "NodeItem") -> None:
        ...
        self.node_config_manager.save_node_states(
            nodes_to_save, graph_zoom_level=current_zoom
        )
    ...
    def unsplit_all_nodes(self, save_state: bool = True) -> None:
        ...
        for node in split_origins:
            try:
                node.split_handler.unsplit_node(
                    save_state=False
                )  # Don't save state for each individual unsplit
                unsplit_count += 1
            except Exception as e:
    ...
    def _update_node_ports(
        self, client_name: str, port_dict: Dict[str, jack.Port]
    ) -> None:
    ...
    def _apply_split_part_visibility(self) -> None:
        ...
        for node in list(self.nodes.values()):
            ...
            input_visible = self.node_visibility_manager.is_input_visible(
                client_name, is_midi=is_midi, tab_type="graph"
            )
            output_visible = self.node_visibility_manager.is_output_visible(
                client_name, is_midi=is_midi, tab_type="graph"
            )

            logger.debug(
                f"Split part visibility for {client_name} (MIDI={is_midi}): input={input_visible}, output={output_visible}"
            )
            ...
            if node.split_input_node:
                ...
                if should_hide != is_currently_hidden:
                    if should_hide:
                        ...
                        self.connection_mgr.update_node_connections_visibility(
                            node.split_input_node, False
                        )
                    else:
                        node.split_input_node.show()
                        self.connection_mgr.update_node_connections_visibility(
                            node.split_input_node, True
                        )
            ...
            if node.split_output_node:
                ...
                if should_hide != is_currently_hidden:
                    if should_hide:
                        ...
                        # Also hide connections for this part
                        self.connection_mgr.update_node_connections_visibility(
                            node.split_output_node, False
                        )
                    else:
                        node.split_output_node.show()
                        self.connection_mgr.update_node_connections_visibility(
                            node.split_output_node, True
                        )
    ...
    def _cleanup_orphaned_unified_sinks(self, all_ports: List[jack.Port]) -> None:
        ...
        """
        unified_sink_manager = getattr(
            self.connection_manager, "unified_sink_manager", None
        )
        if unified_sink_manager:
            ...
            logger.debug(
                f"Available attributes on connection_manager: {[attr for attr in dir(self.connection_manager) if not attr.startswith('_')]}"
            )
            return 0
        ...
        if selected_nodes:
            if key == Qt.Key.Key_H:  # Hide selected nodes
                ...
                for node in selected_nodes:
                    ...
                    if (
                        not node.is_split_origin
                        and not node.is_split_part
                        and node.input_ports
                        and node.output_ports
                    ):
                ...
                for node in selected_nodes:
                    ...
                    if node.is_split_part and node.split_origin_node:
                        node.split_origin_node.split_handler.unsplit_node(
                            save_state=True
                        )
