...
from PyQt6.QtCore import Qt, QPointF, QRectF, QLineF, QEvent, QSize, QTimer
...
class NodeItem(QGraphicsItem):
    ...
    def __init__(
        ...
        ports_to_add: dict | None = None,
        original_client_name: str | None = None,
    ) -> None:
        super().__init__()
        self.client_name = (
            client_name  # This might be the modified name like "Client (Inputs)"
        )
        ...
        self.split_input_node: "NodeItem" | None = (
            None  # Reference to the input part (on origin)
        )
        self.split_output_node: "NodeItem" | None = (
            None  # Reference to the output part (on origin)
        )
        self.split_origin_node: "NodeItem" | None = (
            None  # Reference back to origin (on parts)
        )
        self.original_client_name = (
            original_client_name if original_client_name else client_name
        )
        self.is_folded = (
            False  # New attribute for folding state (primarily for unsplit nodes)
        )
        self._fold_state_initialized_from_config = (
            False  # True after fold state is first set from config
        )
        self._header_rect = (
            QRectF()
        )  # New attribute to store header rect for double-click
        self._internal_state_change_in_progress = (
            False  # Flag to control saving during split/unsplit
        )
            ...
            try:
                if config_manager.node_positions_file.exists():
                    ...
                    if client_config.get(config_manager.IS_INPUT_UNIFIED_KEY, False):
                        ...
                        self.unified_input_sink_name = client_config.get(
                            config_manager.UNIFIED_INPUT_SINK_NAME_KEY
                        )
                        self.unified_input_module_id = client_config.get(
                            config_manager.UNIFIED_INPUT_MODULE_ID_KEY
                        )

                    if client_config.get(config_manager.IS_OUTPUT_UNIFIED_KEY, False):
                        self.is_output_unified = True
                        self.unified_output_sink_name = client_config.get(
                            config_manager.UNIFIED_OUTPUT_SINK_NAME_KEY
                        )
                        self.unified_output_module_id = client_config.get(
                            config_manager.UNIFIED_OUTPUT_MODULE_ID_KEY
                        )
    ...
    def _get_paint_colors(
        ...
        if self.is_split_part and self.split_origin_node:
            ...
            if (
                self.split_origin_node.is_input_unified
                or self.split_origin_node.is_output_unified
            ):
                is_unified = True
        ...
        if is_unified_sink:
            ...
            if (
                "unified-input" in self.client_name
                or "unified_input" in self.client_name
            ):
    ...
    def paint(
        ...
        node_body_bg_color, title_bg_color, final_separator_color, border_color = (
            self._get_paint_colors(option, is_selected)
        )
        ...
        title_rect = QRectF(
            0, 0, self.boundingRect().width(), self._calculated_title_height
        )
        ...
        clip_path.addRoundedRect(
            clip_rect_for_fill, 5, 5
        )  # Use same rounding as border
        ...
        if not self.is_split_origin and not self._is_effectively_folded():
            ...
            actual_body_fill_path = clip_path.intersected(
                body_fill_candidate_path
            )  # Intersect with the rounded shape
            ...
            painter.drawLine(
                int(border_width),
                y_separator,
                int(self.boundingRect().width() - border_width),
                y_separator,
            )
    ...
    def add_port(self, port_name: str, port_obj: Any) -> bool:
        if self.is_split_origin:  # If this is the hidden original node
            ...
            target_node_part = (
                self.split_input_node if is_input_flag else self.split_output_node
            )
            if target_node_part:
    ...
    def remove_port(self, port_name: str) -> bool:
        if self.is_split_origin:  # If this is the hidden original node
            ...
            if self.split_input_node and port_name in self.split_input_node.input_ports:
                ...
                self.split_output_node
                and port_name in self.split_output_node.output_ports
            ):
    ...
    def _calculate_and_set_title_geometry(self, node_width: float) -> float:
        ...
        if not scene or not getattr(scene, "layouter", None):
            raise RuntimeError(
                "Cannot calculate title geometry: No GraphLayouter available"
            )
    ...
    def _hide_all_ports_and_bulk_areas(self) -> None:
        ...
        if not scene or not getattr(scene, "layouter", None):
            raise RuntimeError(
                "Cannot hide ports and bulk areas: No GraphLayouter available"
            )
    ...
    def _show_all_ports_and_bulk_areas(self) -> None:
        ...
        if not scene or not getattr(scene, "layouter", None):
            raise RuntimeError(
                "Cannot show ports and bulk areas: No GraphLayouter available"
            )
    ...
    def _layout_individual_ports(
        ...
        if not scene or not getattr(scene, "layouter", None):
            raise RuntimeError(
                "Cannot layout individual ports: No GraphLayouter available"
            )
    ...
    def _disconnect_all_connections(self) -> None:
        ...
        jack_handler = (
            getattr(current_scene, "jack_connection_handler", None)
            if current_scene
            else None
        )
        if jack_handler:
            try:
                ...
                logger.error(
                    f"Error in _disconnect_all_connections for {client_to_disconnect}: {e}"
                )
    ...
    def _build_context_menu_for_split_part(
        ...
        disconnect_text = (
            "Disconnect all INs"
            if is_input_part
            else ("Disconnect all OUTs" if is_output_part else "Disconnect all")
        )
        action = menu.addAction(disconnect_text)
        action.setEnabled(disconnect_is_enabled)
        if is_input_part:
            action.triggered.connect(self._disconnect_this_part_input_ports)
        elif is_output_part:
            action.triggered.connect(self._disconnect_this_part_output_ports)
        else:
            action.triggered.connect(self._disconnect_all_connections)
        ...
        if self.split_origin_node:
            ...
            unsplit_action.triggered.connect(
                lambda: self.split_origin_node.split_handler.unsplit_node(
                    save_state=True
                )
            )
        else:
        ...
        if is_input_part:
            fold_text = (
                "Unfold Input Part" if self.input_part_folded else "Fold Input Part"
            )
            menu.addAction(fold_text).triggered.connect(
                lambda: self.fold_handler.toggle_input_part_fold()
            )
        elif is_output_part:
            fold_text = (
                "Unfold Output Part" if self.output_part_folded else "Fold Output Part"
            )
            menu.addAction(fold_text).triggered.connect(
                lambda: self.fold_handler.toggle_output_part_fold()
            )
    ...
    def _build_context_menu_for_split_origin(
        ...
        unsplit_action.setShortcut(Qt.Key.Key_U)
        unsplit_action.triggered.connect(
            lambda: self.split_handler.unsplit_node(save_state=True)
        )
    ...
    def _build_context_menu_for_normal_node(
        self, menu: QMenu, disconnect_is_enabled: bool
    ) -> None:
        ...
        if getattr(self, "is_virtual_sink", False) and not getattr(
            self, "is_unified_sink", False
        ):
            menu.addSeparator()
        ...
        split_action.triggered.connect(
            lambda: self.split_handler.split_node(save_state=True)
        )
    ...
    def contextMenuEvent(self, event: QGraphicsSceneContextMenuEvent) -> None:
        ...
        if self.is_split_origin:
            ...
            if self.split_input_node:
                input_part_connections = any(
                    p.connections for p in self.split_input_node.input_ports.values()
                )
            if self.split_output_node:
                output_part_connections = any(
                    p.connections for p in self.split_output_node.output_ports.values()
                )
    ...
    ):  # Type hint is QGraphicsSceneMouseEvent
    ...
    def apply_configuration(self, config: dict) -> None:
        ...
        if self.is_split_origin:
            if (
                self.split_input_node
                and "split_input_pos" in config
                and isinstance(config["split_input_pos"], QPointF)
            ):
                self.split_input_node.setPos(config["split_input_pos"])
            if (
                self.split_output_node
                and "split_output_pos" in config
                and isinstance(config["split_output_pos"], QPointF)
            ):
                self.split_output_node.setPos(config["split_output_pos"])
        elif "pos" in config and isinstance(
            config["pos"], QPointF
        ):  # Node is unsplit (or was never split)
            self.setPos(config["pos"])

        # Apply fold configuration using the fold handler, aware of current split state
        self.fold_handler.apply_fold_config(
            config, self.is_split_origin, self.is_split_part
        )
        ...
        # Apply unified state
        if (
            config.get("is_input_unified")
            or self.is_input_unified
            or config.get("is_output_unified")
            or self.is_output_unified
            or config.get("is_unified")
        ):
    ...
    def itemChange(self, change: QGraphicsItem.GraphicsItemChange, value: Any) -> Any:
        ...
        if change == QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged:
            ...
            if node_to_save_config_for:
                ...
                self.config_manager.save_node_states(
                    {node_to_save_config_for.client_name: node_to_save_config_for}
                )
    ...
    def update_ports(self) -> None:
        ...
        current_ports_in_jack = [
            p
            for p in all_ports_in_jack
            if p.name.startswith(jack_client_name_to_match + ":")
        ]
        ...
        existing_port_item_names = set(self.input_ports.keys()) | set(
            self.output_ports.keys()
        )
        ...
        for port_name in ports_to_add_to_ui:
            ...
            if port_obj:
                self.add_port(
                    port_name, port_obj
                )  # add_port handles delegation for split origins
        ...
        for port_item in list(self.input_ports.values()):  # Iterate copy
            ...
            for conn_item in list(port_item.connections):  # Iterate copy
                if conn_item and conn_item.source_port and conn_item.dest_port:
                    try:
                        jack_handler.break_connection(
                            conn_item.source_port.port_name,
                            conn_item.dest_port.port_name,
                        )
                    except Exception as e:
                        logger.error(
                            f"Error breaking input connection for {port_item.port_name} of {self.client_name}: {e}"
                        )
            ...
            for conn_item in list(port_item.connections):  # Iterate copy
                if conn_item and conn_item.source_port and conn_item.dest_port:
                    try:
                        ...
                        logger.error(
                            f"Error breaking output connection for {port_item.port_name} of {self.client_name}: {e}"
                        )
